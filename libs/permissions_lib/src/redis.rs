...
use bb8_redis::redis::{FromRedisValue, ToRedisArgs};
...
impl RedisPerms {
    ...
    pub async fn insert_rel (
        ...
        let _: () = c.mset(&[(path.to_key(), id.value().as_bytes().to_vec()), (id.to_key(), path.value().as_bytes().to_vec())]).await?;
    ...
    pub async fn remove_rel(
        ...
        let mut c = self.conn.pool.get().await?;
        let id : Vec<u8> = c.get_del(path.to_key()).await?;
        if let Ok(id) = Uuid::from_slice(&id) {
    ...
    pub async fn get_id(
        ...
        let mut c = self.conn.pool.get().await?;
        let id : Vec<u8> = c.get(path.to_key()).await?;
        let id = Uuid::from_slice(&id).ok();
        self.update_rel_lifetime(&mut c, path).await?;
        // panic!("{} {:?}", path.to_key(), id);
        Ok(id)
    }
    ...
    pub async fn get_many_ids(
        ...
        let mut c = self.conn.pool.get().await?;
        let v : Vec<Vec<u8>> = c.mget(paths.iter().map(|p|p.to_key()).collect::<Vec<String>>()).await?;
    ...
    pub async fn remove_many(
        ...
        let ids : Vec<Vec<u8>> = c.mget(&paths_keys).await?;
        let _ : () = c.del(&paths_keys).await?;
        let to_delete = ids.into_iter().filter_map(|x| Uuid::from_slice(&x).ok().and_then(|v| Some(path.construct_id(v).to_key()))).collect::<Vec<String>>();
