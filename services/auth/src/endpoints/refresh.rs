...
use shared::{tokens::jwt::{AccessTokenResponse, RefreshRules, RefreshTokenPayload, RefreshTokenRecord, TokenEncoder}, utils::{app_err::AppErr, header::{get_user_agent, get_user_ip}}, uuid::Uuid};
use tracing::warn;
...
impl RefreshProcessor {
    pub fn begin(
        ...
        let record = state.redis.pop_refresh(refresh_payload.rtid).map_err(|e| return AppErr(e).into_response())?;
        ...
        if !self.refresh_rules.allow_suspicious_refresh {
            if self.record.fingerprint != self.fingerprint ||
                self.record.user_agent != self.user_agent {
                    self.state.send_suspicious_refresh(&self.record.email, self.user_ip.clone(), self.user_agent.clone()).await.map_err(|e| return AppErr(e).into_response())?;
    ...
    pub fn rm_all_refresh(self) -> Result<Self, Response<Body>> {
        self.state.redis.rm_all_refresh(self.refresh_payload.user.clone()).map_err(|e| return AppErr(e).into_response())?;
        Ok(self)
    }

    pub async fn update_refresh_rules(mut self, new_rules: RefreshRules) -> Result<Self, Response<Body>> {
        self.state.update_refresh_rules(&self.record.email, self.user_ip.clone(), self.user_agent.clone(), &new_rules).await.map_err(|e| return AppErr(e).into_response())?;
    ...
    pub fn generate_tokens(self) -> Result<Response<Body>, Response<Body>> {
        let jar = generate_and_put_refresh(self.jar, &self.state, &self.record.user, self.fingerprint, self.user_agent, self.user_ip, self.record.email, self.refresh_payload.rules).map_err(|e| return AppErr(e).into_response())?;
        let access_response = generate_access(self.record.user).map_err(|e| return AppErr(e).into_response())?;
